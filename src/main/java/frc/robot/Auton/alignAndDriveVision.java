...
public class alignAndDriveVision extends CommandBase {
    ...
    public alignAndDriveVision(double xCoord, double baseSpeed, double slowSpeed, double tolerance) {
        ...
        if (delayTimer.get() > 0.5) {
            ...
            if (drive.getRobotPos().getRotation().getDegrees() < 10) {
                if (timer.get() > 0.2) {
                    ...
                    error = Math.max(3, Math.min(10, error));
                    double dir = 1;
                    if (sigError > 0)
                        dir = -1;
                    drive.velX = dir * error / 15;
                    drive.omega = 0;
                }
            } else {
                ...
                double tarTheta = 0;
                if (Math.abs(dThetaRegErr) > Math.abs(dThetaPosErr)
                        && Math.abs(dThetaNegErr) > Math.abs(dThetaPosErr)) {
